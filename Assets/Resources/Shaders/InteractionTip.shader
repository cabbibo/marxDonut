  ...
  SubShader {
    ...
    Pass {
      ...
      float2 map( in float3 pos ){
        ...
        res.x = lerp( res.x , sdBox( pos , float3( .2 , .2 , .2 ) ) , 1 - _TriggerDown);


        res.x -= ((.1 * _TriggerDown) + _Hovered * .04)* noise( pos * 10 );
        res.y = 1;
      ...
      fixed4 frag(VertexOut i) : COLOR {
        ...
        col = pow(col,  2.2);  
           float3 noMoonCol = col;
                                ...
                                col = lerp( fullMoonCol , noMoonCol , 1 );
        float3 col2 = float3( length( col ) , length( col ) , length( col ) ) * .5;

        col = lerp( col2 , col , _TriggerDown );
